# Movie Rating Predictor — Historical Project Context

> **Superseded planning record.** This document preserves the original project
> vision, early enrichment-first roadmap, and historical phase numbering. Its
> implementation status and "what to do next" section are not current. Use the
> [implementation plan](../IMPLEMENTATION_PLAN.md) for current status, the
> [feature dictionary](../FEATURE_DICTIONARY_V1.md) for the accepted PRD feature
> contract, and the [documentation index](../README.md) for canonical sources.
